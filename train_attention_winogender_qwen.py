...
with open(outputfile, "w", newline="") as f:
    ...
    for step in range(num_steps):
        ...
        for bb in range(0, len(batch_texts), batch_size):
            ...
            # If your MyAttentionWrapper stores attn_override on GPU
            for i, layer in enumerate(model.model.layers):
                layer.self_attn.attn_override = None   # **MUST** drop the tensor handle
            ...
            print(f"[mem‑MB]  alloc={torch.cuda.memory_allocated()//1e6}  \
            peak={torch.cuda.max_memory_allocated()//1e6}")
            del batch_outputs, attention_weights
            del original_attention_weights, conterfactual_attention_weights
            torch.cuda.empty_cache()          # **MUST**
        ...
        del odds_all, odds_average



        print(f"eval: [mem‑MB]  alloc={torch.cuda.memory_allocated()//1e6}  \
        peak={torch.cuda.max_memory_allocated()//1e6}")
