...
### for add a space before punctuation symbols
import re
def add_space_before_punct(text):
    # Add a space before punctuation symbols
...
class Intervention():
    ...
    def __init__(self,
        ...
        # print(self.base_strings)
        # Tokenized bases
        self.base_strings_tok = [
            self.enc.encode(add_space_before_punct(s), add_special_tokens=False)
        ...
        self.candidates = []
        for c in candidates:
            # 'a ' added to input so that tokenizer understands that the first word follows a space
            modified_text = add_space_before_punct('a ' + c)
            tokens = self.enc.tokenize(modified_text)[1:]  # Remove the first token corresponding to 'a'
...
class Model():
    ...
    def __init__(self,
        ...
        assert (self.is_gpt2 or self.is_roberta)
    ...
    def neuron_intervention(self,
        ...
        for layer in set(layers):
            # if layer not in rep:
            #     print(f"Layer {layer} not found in rep. Available layers are: {list(rep.keys())}")
            #     print(layers)
    ...
    def neuron_intervention_experiment_GD(self,
                                       word2intervention,
                                       intervention_type,
                                       layers_to_adj=[],
                                       neurons_to_adj=[],
        ...
        for word in tqdm(word2intervention, desc='words'):
            word2intervention_results[word] = self.neuron_intervention_single_experiment_GD(
                word2intervention[word], intervention_type, layers_to_adj, neurons_to_adj,
                alpha, intervention_loc=intervention_loc)
            print(word2intervention_results[word])

        return word2intervention_results


    def neuron_intervention_single_experiment_GD(self,
                                              intervention,
        ...
        with torch.no_grad():
            ...
            woman_representations = self.get_representations(
                ...
                intervention.position)
            # print("base representations")
            # print(base_representations.keys())
            # print("man representations")
            # print(man_representations.keys())
            # print("woman representations")
            # print(woman_representations.keys())
            #[13,768] why 13?
            print("get the representations")
            ...
            if intervention_type == 'man_indirect':
