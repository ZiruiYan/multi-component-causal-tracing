...
from utils.utils_attention import accelerated_candidate_probs, get_probabilities_for_examples_multitoken_batch, get_probabilities_for_examples_multitoken_batch_grad, CounterfactualAttentionGPT2, evaluate
...
parser.add_argument("--split", type=str, help = 'dev/test', default='dev', choices=['dev', 'test'])
parser.add_argument("--model", type=str, help = 'model type', default='gpt2', choices=['distilgpt2', 'gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl'])
...
parser.add_argument("--cache_dir", type=str, help='model cache dir', default=None)
...
directory_name = f"results/Attention_winobias/Ours/{current_time}/{args.model}_l1_{args.lambda1}_l2_{args.lambda2}_scheduling_{method}_lr_{args.lr}_trail_{args.trail}"
...
for i, layer in enumerate(model.transformer.h):
    original_attentions.append(layer.attn)
...
batch_candidates = [
    ...
    for example in examples
    for candidate in [
        example.female_occupation_continuation, 
        example.male_occupation_continuation
    ]
]
