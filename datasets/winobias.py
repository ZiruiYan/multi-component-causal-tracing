...
from utils.experiment import Model, Intervention

DATA_DIR = Path(__file__).resolve().parents[1] / "data" / "winobias"
...
def load_dev_examples(path=DATA_DIR, verbose=False):
    return load_examples(path, 'dev', verbose)

def load_test_examples(path=DATA_DIR, verbose=False):
    return load_examples(path, 'test', verbose)

def load_examples(path, split, verbose=False):
    path = Path(path)
    print(f'Split: {split.upper()}')
    with open(path / 'female_occupations.txt') as f:
        female_occupations = [row.lower().strip() for row in f]
    with open(path / 'male_occupations.txt') as f:
