...
from utils.experiment import Intervention

DATA_DIR = Path(__file__).resolve().parents[1] / "data" / "winogender"


def load_examples(path=DATA_DIR):
    path = Path(path)
    bergsma_pct_female = {}
    bls_pct_female = {}
    with open(path / 'winogender_occupation_stats.tsv') as f:
    ...
    examples = []
    with open(path / 'winogender_templates_structurefilter.tsv') as f:
        next(f, None)  # skip the headers
        row_pair = []
        for row in csv.reader(f, delimiter='\t'):
            row_pair.append(row)
